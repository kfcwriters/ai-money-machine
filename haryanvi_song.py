...
def assemble_video(lines, video_paths, audio_path):
    ...
    for vp in video_paths:
        if Path(vp).exists():
            loaded.append(VideoFileClip(vp).without_audio().resized(height=VIDEO_HEIGHT))
    black = VideoFileClip("fallback_black.mp4").without_audio().resized(new_size=(VIDEO_WIDTH, VIDEO_HEIGHT))
    for i, line in enumerate(lines):
        v = loaded[i % len(loaded)] if loaded else black

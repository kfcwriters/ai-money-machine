...
def generate_intro(title):
    prompt = f"""Write a 1‑sentence compelling intro for a Dev.to article titled "{title}" about nutrition and diet. Keep it under 100 characters."""
...
def main():
    ...
    if not title:
        logging.error("No published posts found on NutriAid Hashnode. Ensure the SEO article ran first.")
        sys.exit(1)

    intro = generate_intro(title)
    body = f"*Originally published on [NutriAid Blog]({link})*\n\n{intro}\n\n{content}\n\n---\n*Need personalised diet advice? Visit [NutriAid]({HIRE_ME_URL}).*"
